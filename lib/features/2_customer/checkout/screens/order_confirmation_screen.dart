...
class OrderConfirmationScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Konfirmasi Pesanan', style: TextStyle(fontWeight: FontWeight.bold)), backgroundColor: Color(0xFF88D66C)),
      body: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          children: [
            const Icon(Icons.check_circle_outline, color: Colors.lightBlue, size: 120),
            const SizedBox(height: 20),
            const Text('Terima kasih atas pesanan Anda!',
                style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Text('Metode Pembayaran: $paymentMethod', style: const TextStyle(fontSize: 16)),
                ...
                children: [
                  ...
                  ...cartItems.map(
                    (item) => ListTile(
                      ...
                      trailing: Text('${item['quantity']} x Rp ${item['price']} = Rp ${item['quantity'] * item['price']}'),
                    ),
                  ),
                  const Divider(),
                  ListTile(
